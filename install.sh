...
PLATFORM_CHOICE_FILE="$BACKUP_DIR/.platform_choice"
...
do_install() {
    ...
    echo ""
    echo "Setting up for $shell_choice on $platform_name..."
    echo ""
    ...
    # Generate ~/.homebrew_env with the correct brew path
    echo "# Generated by dotfiles install.sh — do not edit" > "$HOMEBREW_ENV_FILE"
    echo "eval \"\$($brew_path shellenv)\"" >> "$HOMEBREW_ENV_FILE"
    echo "  created  ~/.homebrew_env ($platform_name)"

    # Save choices
    echo "$shell_choice" > "$SHELL_CHOICE_FILE"
    echo "$platform_name" > "$PLATFORM_CHOICE_FILE"
...
do_status() {
    ...
    if [ -f "$PLATFORM_CHOICE_FILE" ]; then
        echo "  Platform: $(cat "$PLATFORM_CHOICE_FILE")"
    fi
    echo ""
...
do_rollback() {
    ...
    if [ -f "$HOMEBREW_ENV_FILE" ]; then
        rm "$HOMEBREW_ENV_FILE"
        echo "  removed  ~/.homebrew_env"
    fi
